"""Weiss Schwarz Barcelona Data"""

from src.services.common.value_object.author_value_object import AuthorValueObject


# pylint: disable=too-few-public-methods
class WeissSchwarzBarcelonaData:
    """Weiss Schwarz Data."""
    _AUTHOR = AuthorValueObject(
        name='Weiß Schwarz - Barcelona',
        url='https://www.facebook.com/106532907475749',
        icon_url="https://scontent.xx.fbcdn.net/v/t1.0-1/cp0/p50x50/79356428_107588690732405_4729016489070297088_n.png?"
        "_nc_cat=108&_nc_sid=dbb9e7&_nc_oc=AQnVZD9cFjH5bgu9iGsSIf5Y-X0l8VOW2YkqETVUcPPAZ_1nYAVlmfVHdnaQFXWeIxo&_nc_ht="
        "scontent.xx&oh=f5fa2e8d7b1c093d81a5fc9efad41f66&oe=5E9399AB")
